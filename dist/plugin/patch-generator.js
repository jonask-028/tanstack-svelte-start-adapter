...
\t\t\t};
\t\t\t`;
function patchRouterGeneratorTemplate() {
    try {
        let dir = process.cwd();
        const templatePaths = [];
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const standardPath = resolve(dir, "node_modules/@tanstack/router-generator/dist/esm/template.js");
                ...
                try {
                    ...
                    for (const entry of entries) {
                        if (entry.startsWith("@tanstack+router-generator@")) {
                            const bunPath = join(bunDir, entry, "node_modules/@tanstack/router-generator/dist/esm/template.js");
